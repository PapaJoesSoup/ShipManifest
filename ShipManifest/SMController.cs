...
namespace ShipManifest
{
  internal class SMController
  {
    ...
    /// <summary>
    /// SMController
    /// Object Scope:  Instantiated for theCurrent or selected vessel by the controller within the current scene, if allowed
    /// </summary>
    ...
    internal static void DumpPartCrew(Part part)
    {
    ...
    internal static bool IsPartCrewFull(Part part)
    {
      if (DFInterface.IsDFInstalled && part.Modules.Contains("DeepFreezer"))
      {
        ...
        return ((IDeepFreezer)deepFreezer).DFIPartFull;
      }
