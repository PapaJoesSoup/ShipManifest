...
namespace ShipManifest
{
  internal static class WindowManifest
  {
    ...
    internal static bool ShowWindow
    {
      ...
      set
      {
        if (!value && SMAddon.smController != null)
    ...
    internal static void Display(int windowId)
    {
      ...
      try
      {
        ...
        if (SMAddon.smController.IsRecoverable)
        {
          PreLaunchGUI();
        }
        ...
        SMAddon.RepositionWindow(ref WindowManifest.Position);
      }
      catch (Exception ex)
      {
    ...
    private static void ResourceButtonList()
    {
      try
      {
        foreach (string resourceName in SMAddon.smController.PartsByResource.Keys)
        {
          GUILayout.BeginHorizontal();
          int width = 273;
          if ((!SMSettings.RealismMode || SMAddon.smController.IsRecoverable) && resourceName != "Crew" && resourceName != "Science")
            width = 185;
      ...
      try
      {
        ...
        if (SMAddon.smController.SelectedResources.Count > 0)
        {
          foreach (Part part in SMAddon.smController.SelectedResourcesParts)
          {
            ...
            {
              GUILayout.BeginHorizontal();
              GUILayout.Label(string.Format("{0}, ({1}/{2})", part.partInfo.title, Utilities.GetPartCrewCount(part).ToString(), part.CrewCapacity.ToString()), GUILayout.Width(265), GUILayout.Height(20));
              GUILayout.EndHorizontal();
            }
