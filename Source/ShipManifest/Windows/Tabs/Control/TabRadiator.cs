      ...
      try
      {
        ...
        while (iRadiators.MoveNext())
        {
          ...
          if (Event.current.type == EventType.Repaint && rect.Contains(Event.current.mousePosition))
            SMHighlighter.SetMouseOverData(rect, scrollY, scrollX, WindowControl.TabBox.height + WindowControl.HeightScale, iRadiators.Current.SPart, Event.current.mousePosition);
        }
        iRadiators.Dispose();
